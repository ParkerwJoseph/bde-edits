...
from shared.services.chunking.prompts import get_prompt_manager
...
class ChunkingService:
    ...
    async def process(
        ...
        # Execute chunking strategy with progress callback
        chunks = await strategy.execute(normalized, input, progress_callback)
    ...
    async def _generate_embeddings(self, chunks: List[ChunkOutput]) -> List[ChunkOutput]:
        ...
        try:
            ...
            logger.info(f"[ChunkingService] Generated {len(embeddings)} embeddings in batch")
            ...
            logger.error(f"[ChunkingService] Batch embedding generation failed: {e}")
            # Set all embeddings to None on failure
            for chunk in chunks:
                chunk.embedding = None
