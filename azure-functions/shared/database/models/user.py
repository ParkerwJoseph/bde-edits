...
class User(SQLModel, table=True):
    ...
    azure_oid: str = Field(index=True, max_length=255)
    azure_tid: str = Field(index=True, max_length=255)
    tenant_id: Optional[str] = Field(default=None, foreign_key="tenants.id", index=True)
    # Note: role_id foreign key removed to avoid importing Role model
