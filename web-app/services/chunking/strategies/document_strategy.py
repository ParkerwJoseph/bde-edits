...
from services.chunking.models import ChunkInput, ChunkOutput, NormalizedInput, SourceType
from services.chunking.prompts import PromptManager
from database.models.document import BDEPillar, ChunkType
...
class DocumentChunkingStrategy(BaseStrategy):
    ...
    async def execute(
        ...
        logger.info(f"[DocumentStrategy] Processing {len(content_units)} pages")
        ...
        for unit_idx, unit in enumerate(content_units):
            ...
            logger.info(f"[DocumentStrategy] Processing page {page_num}/{len(content_units)}")
    ...
    def _build_llm_content(self, unit: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        For pages with detected tables, includes both the image and the
        structured table data so the LLM can accurately extract tabular content
        with preserved column-header relationships.
        """
        content = []
        ...
        if unit.get("image_base64"):
            ...
            if table_data:
                # Page has structured table data — provide it alongside the image
                # so the LLM has accurate column-row relationships
                tables_text = "\n\n".join(
                ...
                content.append({
                    "type": "text",
                    "text": (
                        f"[This is Page {unit.get('unit_number', 1)}]\n\n"
                        f"IMPORTANT: This page contains structured tabular data. "
                        f"The following tables have been extracted with accurate column-header relationships. "
                        f"Use this structured data as the PRIMARY source for extracting metrics and values "
                        f"(it preserves the exact column-row mapping that may be hard to read from the image alone). "
                        f"Use the image for any non-table content (titles, notes, narrative text).\n\n"
                        f"{tables_text}"
                    )
                })
            else:
