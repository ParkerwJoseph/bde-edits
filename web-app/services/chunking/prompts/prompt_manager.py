...
from database.models.document import PILLAR_DESCRIPTIONS
...
Group all content related to the same pillar—including sentences, metrics, tables, lists, and supporting context—into a single chunk per pillar per page, regardless of length.
Never split semantically connected sentences, metrics, tables, lists, or descriptions of multiple people/entities if they all support the same pillar concept.
Only create a new chunk when the pillar, primary business question, or analytical focus changes significantly.
...
**STRUCTURED TABLE DATA HANDLING**
When structured table data is provided alongside the page image (prefixed with "[Extracted Table]"):
- Use the structured table as the PRIMARY source for all tabular values — it has accurate column-header mappings.
- For financial tables with time periods as columns (e.g., monthly, quarterly, yearly data):
  - Preserve the COMPLETE time-series data for each line item with all period labels and values.
  - Format as: "Line Item: Period1=Value1, Period2=Value2, ..., Total=ValueN"
  - Example: "Hardware Sales: Dec 2023=10,995.80, Jan 2024=3,408.97, Feb 2024=6,638.00, ..., Total=91,934.65"
  - NEVER output just a list of numbers without their corresponding period labels.
- Keep all rows from the same financial section (e.g., all REVENUE items, all COST OF SALES items) in a single chunk.
- Use the image for context like section headers, footnotes, and narrative text that may not appear in the extracted table.
...
- Tables: Extract ALL rows, columns, headers, and values with their column labels preserved
  ...
  "chunks": [
    {{{{
      ...
      "metadata": {{{{
        ...
        "data_type": "metrics|narrative|time_series|mixed",
        "has_metrics": true,
        "time_period": "Exact period as stated in document (e.g., FY, Q1-Q4, month, date range), null if not mentioned",
        "period_columns": ["List of period column headers if this is time-series data, e.g., ['Dec 2023', 'Jan 2024', ...], null otherwise"],
...
1. CUSTOMER BASE SUMMARY: Total customers, active vs inactive, new acquisitions
2. SEGMENT ANALYSIS: Distribution by size/industry/geography if available
3. HEALTH INDICATORS: Outstanding balances, payment behavior patterns

Example good chunk:
"The company has 145 total customers, with 128 active (88% retention).
Average customer balance is $12,500. Top 10 customers represent 65% of total AR.
Most customers are in manufacturing (45%) and distribution (30%) sectors."
""",
